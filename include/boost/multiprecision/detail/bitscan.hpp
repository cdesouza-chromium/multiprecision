...
{
   return static_cast<std::size_t>(__builtin_ctzl(static_cast<unsigned int>(mask)));
}
BOOST_FORCEINLINE std::size_t find_lsb(unsigned long long mask, std::integral_constant<int, 3> const&)
{
   return static_cast<std::size_t>(__builtin_ctzll(static_cast<unsigned int>(mask)));
}
BOOST_FORCEINLINE std::size_t find_msb(std::size_t mask, std::integral_constant<int, 1> const&)
{
   return static_cast<std::size_t>((static_cast<int>(sizeof(unsigned)) * CHAR_BIT) - 1 - __builtin_clz(static_cast<unsigned int>(mask)));
}
BOOST_FORCEINLINE std::size_t find_msb(unsigned long mask, std::integral_constant<int, 2> const&)
{
   return static_cast<std::size_t>((static_cast<int>(sizeof(unsigned long)) * CHAR_BIT) - 1 - __builtin_clzl(static_cast<unsigned int>(mask)));
}
BOOST_FORCEINLINE std::size_t find_msb(unsigned long long mask, std::integral_constant<int, 3> const&)
{
   return static_cast<std::size_t>((static_cast<int>(sizeof(unsigned long long)) * CHAR_BIT) - 1 - __builtin_clzll(static_cast<unsigned int>(mask)));
}
