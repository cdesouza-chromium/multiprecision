...
inline void copy_and_round(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>& res, Int& arg, std::ptrdiff_t bits_to_keep = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count)
{
   ...
   if (static_cast<std::ptrdiff_t >(bits_to_keep) > msb + 1)
   {
      ...
      res.bits() = arg;
      eval_left_shift(res.bits(), static_cast<unsigned int>(bits_to_keep - msb - 1));
   ...
   else if (static_cast<std::ptrdiff_t>(bits_to_keep) < msb + 1)
   {
      ...
      bool roundup = eval_bit_test(arg, static_cast<unsigned int>(msb - bits_to_keep));
      // Then check for a tie:
      if (roundup && (msb - bits_to_keep == static_cast<std::ptrdiff_t>(eval_lsb(arg))))
      {
         // Ties round towards even:
         if (!eval_bit_test(arg, static_cast<unsigned int>(msb - bits_to_keep + 1)))
            roundup = false;
      }
      // Shift off the bits_to_keep we don't need:
      eval_right_shift(arg, static_cast<unsigned int>(msb - bits_to_keep + 1));
      ...
      if (roundup)
      {
         ...
         if (bits_to_keep)
         {
            if (eval_bit_test(arg, static_cast<unsigned int>(bits_to_keep)))
            {
      ...
      if (bits_to_keep != cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count)
      {
         ...
         // to narrower types:
         eval_left_shift(arg, static_cast<unsigned int>(static_cast<std::ptrdiff_t>(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count) - bits_to_keep));
   ...
   const BinFloat2& a, const BinFloat3& b)
{
   ...
   bool s = a.sign();
   dt     = a.bits();
   if (a.exponent() > static_cast<exponent_type>(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count) + b.exponent())
   {
   ...
   else
   {
      ...
      eval_left_shift(dt, static_cast<unsigned int>(e_diff));
...
#pragma warning(disable : 6326) // comparison of two constants
   ...
   else
   {
      ...
      res.exponent() -= static_cast<Exponent>(lshift);
...
#pragma warning(disable : 6326) // comparison of two constants
   ...
   eval_left_shift(t, static_cast<unsigned int>(gb + 1));
   ...
   else
   {
      ...
      constexpr const unsigned lshift = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count < limb_bits ? 2 : limb_bits;
      eval_left_shift(q, lshift);
      res.exponent() -= static_cast<typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type>(lshift);
      eval_left_shift(r, 1u);
